...
/* Modified: Mike Santoro, Date: 11/05/2012, Purpose: updated variables for secure environment and added aditional exclusions */
;(function($){
 var _ = {
  build: {
   init: function(){
  ...
  listeners: {
   ...
   leavePage: function(){
    var $obj = $uploadButton,
        $window = $(window);
    $jForm
     .find("input[type=button],input[onclick*=Submit],input[value*=Submit],input[value*=Save],input[type=submit],select[onchange*=Submit].selExclude")
      .not($obj)
  ...
  general: {
   callUpload: function($obj){
      ...
      if(objData.svrstatus === "0"){
       $obj
        ...
        .next("div")
         .remove();
       $jForm
        .find("select[onchange*=Submit].selExclude")
        .unbind();
    ...
    $jForm
     .attr({
      action: "index.cfm?action=santoro.services.fileUploadAJAX&_nhd&fileField=" + jFile,
      method: "POST",
   ...
   callDelete: function(fileList, cleaner){
    $.ajax({
     url: "index.cfm?action=santoro.services.fileDeleteAJAX",
     type: "POST",
