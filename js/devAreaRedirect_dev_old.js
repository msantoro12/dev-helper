...
#dumpData .superDropDown ul li img{\
 ...
 <div id="dumpData">\
  <div id="dumpMenu">\
   ...
   <span id="images" class="superDropDown expandable">&nbsp;| <span style="float:none;">Images</span><ul id="imgList"></ul></span>\/*<span id="log"></span>\*/
 ...
 $(document).ready(function(){
  $('body').append(devMenu);
  controllerName = getCookie("controllerName") || "";
  depScript = getCookie("depScript") || "";
  $("#controllerName")
   .val(controllerName)
   .bind("change", function(){
    setCookie("controllerName", $(this).val());
   });
  $('#depScript')
   .val(depScript)
   .bind("change", function(){
    setCookie("depScript", $(this).val());
    if($(this).val()){
     loadScript($(this).val());
    }
   });
  if(depScript){
   loadScript(depScript);
  }
  $("#images span")
   .bind("click", function(){
    buildImageList();
  ...
  $('#remRed').change(function(){
   if(!$(this).attr("checked")){
    $act = 'act.replace(/[A-Za-z]+\./, controllerName + ".")';
    $act2 = 'act.replace(/action=[A-Za-z]+\./, "action=" + controllerName + ".")';
    $('form').each(function(){
     var $obj = $(this);
     $obj.data('prevVal', $obj.attr('action'));
     $obj.attr('action', $obj.attr('action').replace(/action=[A-Za-z]+\./, "action=" + controllerName + "."));
    });
   }
   else{
    $('form').each(function(){
  ...
  $('body').delegate('a', 'click', function(event){
   ...
   if($obj.attr('href') != null && $obj.attr('href') != 'javascript:void(0);' && $obj.attr('href') != '#' && !$('#remRed').attr("checked")){
    $obj
     ...
     .replace(/action=[A-Za-z]+\./, "action=" + controllerName + "."));
   }
  });
