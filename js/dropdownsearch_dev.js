...
$.getScript('//dropdown-search.googlecode.com/svn/js/jquery.autocomplete.js', function(){
 var actMenus = [];
 $("head").append('<link rel="stylesheet" type="text/css" href="http://dropdown-search.googlecode.com/svn/styles/dropdown.css?' + Math.floor(Math.random()*1100) + '">');
    ...
    //$obj
     ...
     //.parent()
      //.ajaxSuccess(function(event){
       //buildQArea($(this).find(".ac_select"), eval($(this).find(".ac_select")));
       //acBindings();
       //$(this).unbind(event);
      //});
 });
...
function buildQArea(obj, self){
 var $obj = obj,
     ...
     searchTerms = $obj.children().map(function() {return $(this).html().replace(/\"/g, "");}).get(),
 ...
 $('input.ac_input').each(function(e){
  ...
  if(!isBound){
   ...
   .result(function(event, data, formatted){
    if(data){
     $selObj.find("option").filter(function(){
